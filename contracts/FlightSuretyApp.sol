...
contract FlightSuretyApp {
    ...
    modifier canAirlineCreateOrUpdate() {
        bool canCreate = (dataContract.getAirlinesCount() == 0) ||
            (dataContract.isAirline(msg.sender) == true);
    ...
    function voteAirline(address airlineAddress)
        ...
        canAirlineCreateOrUpdate()
    {
        bool needApproved = dataContract.voteAirline(
            airlineAddress,
            msg.sender
        );
    ...
    function processFlightStatus(
        ...
        if (statusCode == STATUS_CODE_LATE_AIRLINE) {
            ...
            uint256 insurancePayoutValue = dataContract.getInsurancePayoutValue(
                key
            );
            uint256 passengerBalance = dataContract.getPassengerBalance(
    ...
    function withdrawPassengerFunds() public requireIsOperational() {
        ...
        require(
            passengerBalance > 0,
            "Insufficient funds on passenger's balance"
        );
        dataContract.pay(msg.sender);
        emit UpdatedPassengerBalance(
            dataContract.getPassengerBalance(msg.sender)
        );
    }
