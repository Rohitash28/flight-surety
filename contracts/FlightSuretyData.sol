...
contract FlightSuretyData {
    ...
    uint256 public INSURANCE_STATUS_UNKNOWN = 0;
    uint256 public INSURANCE_STATUS_IN_PROGRESS = 1;
    uint256 public INSURANCE_STATUS_PAID = 1;
    uint256 public INSURANCE_STATUS_CLOSED = 2;
    ...
    modifier checkAirlineApproved(address airlineAddress) {
        ...
        require(
            (airline.needApprove == false) ||
                (airline.votes.votersCount >= airline.minVotes),
            "Need approval from other Airlines"
        );
        _;
    }
    ...
    {
        require(
            airlines[airlineAddress].votes.voters[voterAddress] == false,
            "Airline already voted by this account"
        );

        airlines[airlineAddress].votes.votersCount = airlines[airlineAddress]
            .votes
            .votersCount
            .add(1);
        airlines[airlineAddress].votes.voters[voterAddress] = true;

        airlines[airlineAddress].needApprove =
            airlines[airlineAddress].votes.votersCount <
            airlines[airlineAddress].minVotes;
    ...
    function creditInsurees(bytes32 flightKey) external requireIsOperational {
        InsuranceInfo storage insurance = insurances[flightKey];
        if (insurance.status == INSURANCE_STATUS_IN_PROGRESS) {
            ...
            passengerBalances[insurance.passenger] = balance.add(
                insurancePayoutValue
            );
    ...
    function getInsurancePayoutValue(bytes32 flightKey)
        public
        view
        requireIsOperational
        returns (uint256)
