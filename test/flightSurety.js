...
contract("Flight Surety Tests", async (accounts) => {
  ...
  it("(airline) vote airlines", async () => {
    ...
    try {
      await config.flightSuretyApp.voteAirline(airlineToVote, {
        from: config.firstAirline,
      });
    } catch (e) {
      console.log(e);
  ...
  it("(airline) fund airlines", async () => {
    ...
    const resultSuccess = await config.flightSuretyData.getAirline.call(
      accounts[7]
    );
    assert.equal(
      resultSuccess.isFunded,
      true,
      "Should funded approved airline"
    );
  });
